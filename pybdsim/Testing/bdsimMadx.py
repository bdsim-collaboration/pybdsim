    ...
    def Compare(self, plot='all', addPrimaries=False, showPlots=False, showResiduals=True, rebdsim="rebdsim", noPlots=False):
        ...
        for opt in plotopts:
            ...
            if (opt=='beta'):
                ...
                B_optfn_x    = bdata.Alpha_x()
                B_optfn_y    = bdata.Alpha_y()
                B_opterr_x   = bdata.Sigma_Alpha_x()
                B_opterr_y   = bdata.Sigma_Alpha_y()
        ...
        if showResiduals:
            ...
            if(self.nparticles<600):
                ...
                _plt.xticks(locs, ["%2.1e" % x for x in locs])
                locs,labels = _plt.yticks()
                _plt.yticks(locs, ["%2.1e" % x for x in locs])
