...
def Xsuite2Gmad(inputline, outputfilename, tws0,
    ...
    **Xsuite2Gmad** convert an Xsuite line into a gmad tfs file for bdsim
    ...
    if ("LimitRect" in types or "LimitEllipse" in types) and not collimatordict:
    ...
    for name in line.element_names[startindex:endindex][::stepsize]:
        ...
        elif aperlocalpositions and name in aperlocalpositions:  # split aperture if provided
            elements_split_with_aper = _GetElementSplitByAperture(gmadElement, aperlocalpositions[name])
            for ele in elements_split_with_aper:
                machine.Append(ele)
        else:  # Get element with single aperture
            element_with_aper = _GetSingleElementWithAper(name, item, gmadElement, aperturedict,
...
def _Xsuite2GmadElementFactory(name, item, allelementdict, verbose, userdict, collimatordict, partnamedict,
    ...
    #######################################################################
        ...
        angle = item.angle
        e1 = item.edge_entry_angle
        e2 = item.edge_exit_angle
        k1 = item.k1
        if e1 != 0:
            kws['e1'] = e1
        if e2 != 0:
    ...
    #######################################################################
    elif Type in {_xt.LimitRect, _xt.LimitEllipse}:
...
def _GetSingleElementWithAper(name, item, gmadElement, aperturedict, defaultAperSize, defaultAperShape):
    ...
    if name in aperturedict:
        ...
        this_aperdict = {'APER_1': defaultAperSize}
...
def Xsuite2GmadBeam(line, tws0, startindex=0, verbose=False, extraParamsDict={}):
    ...
    beamparams = {"SetBetaX": 'betx', "SetBetaY": 'bety', "SetAlphaX": 'alfx', "SetAlphaY": 'alfy',
